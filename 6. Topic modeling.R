...
ap_top_terms <- ap_topics %>%    # from the ap_topics data set
  ...
  arrange(topic, -beta)          # arrange them first by topic and then by beta in descending order   
...
beta_spread %>%                                            # from the beta_spread data set
  ...
  ggplot(aes(reorder(term, log_ratio), log_ratio)) +       # plor term vs log_ratio
  geom_bar(stat = "identity") +                            # specift bars as the type of plot
...
tidy(AssociatedPress) %>%                       # from the AssociatedPress data set (applying the tidy() function)
  filter(document == 6) %>%                     # filer for the "document" title to be equal to 6
  arrange(desc(count))                          # arrange it in descending order by count  
...
top_terms <- chapter_topics %>%     # from the chapter_topics data set
  ...
  ungroup() %>%                     # ungroup them
  arrange(topic, -beta)             # arrange them by beta in descending order
top_terms
...
chapters_gamma <- chapters_gamma %>%                                     # from the chapters_gamma data set
  separate(document, c("title", "chapter"), sep = "_", convert = TRUE)   # separate the "document" cloum into two new columns ("title" and "chapter")
chapters_gamma
...
# Compare each of the "consensus" topic for each book (the most common topic
# among its chapters), and see which were most often misidentified
...
books_topics <- chapter_classifications %>%      # from the chapter_classifications data set
  ...
  ungroup() %>%                                  # ungroup them
  transmute(consensus = title, topic)            # transform the data set intoa tibble with column names "consensus" and "topic"

chapter_classifications %>%                      # from the chapter_classifications data set
  inner_join(books_topics, by = "topic") %>%     # perform in inner join with the books_topics data set by topic
...
# Find which words were incirrectly classified
...
wrong_words %>%                                     # from the wrong_words data set
  ...
  ungroup() %>%                                     # ungroup them
  arrange(desc(n))                                  # arrange n in descending order
