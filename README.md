)# Text Mining with R

This repository contains codes, perosnal notes and exercises from the book 'Text Mining with R' written by Julia Silge & David Robinson.

## Requirements

* [R](https://www.r-project.org/)

## Packages

* [broom](https://cran.r-project.org/web/packages/broom/broom.pdf)
* [dplyr](https://cran.r-project.org/web/packages/dplyr/dplyr.pdf)
* [forcats](https://cran.r-project.org/web/packages/forcats/forcats.pdf)
* [ggplot2](https://cran.r-project.org/web/packages/ggplot2/ggplot2.pdf)
* [ggraph](https://cran.r-project.org/web/packages/ggraph/ggraph.pdf)
* [gutenbergr](https://cran.r-project.org/web/packages/gutenbergr/gutenbergr.pdf)
* [igraph](https://cran.r-project.org/web/packages/igraph/igraph.pdf)
* [janeaustenr](https://cran.r-project.org/web/packages/janeaustenr/janeaustenr.pdf)
* [mallet](https://cran.r-project.org/web/packages/mallet/mallet.pdf)
* [matrix](https://cran.r-project.org/web/packages/Matrix/Matrix.pdf)
* [lubridate](https://cran.r-project.org/web/packages/lubridate/lubridate.pdf)
* [purrr](https://cran.r-project.org/web/packages/purrr/purrr.pdf)
* [readr](https://cran.r-project.org/web/packages/readr/readr.pdf)
* [reshape](https://cran.r-project.org/web/packages/reshape2/reshape2.pdf)
* [scales](https://cran.r-project.org/web/packages/scales/scales.pdf)
* [stringr](https://cran.r-project.org/web/packages/stringr/stringr.pdf)
* [tidyr](https://cran.r-project.org/web/packages/tidyr/tidyr.pdf)
* [tidytext](https://cran.r-project.org/web/packages/tidytext/tidytext.pdf)
* [tm](https://cran.r-project.org/web/packages/tm/tm.pdf)
* [tm.plugin.webmining](https://cran.r-project.org/web/packages/tm.plugin.webmining/tm.plugin.webmining.pdf)
* [topicmodels](https://cran.r-project.org/web/packages/topicmodels/topicmodels.pdf)
* [wordclod](https://cran.r-project.org/web/packages/wordcloud/wordcloud.pdf)

## Book Website

* [Text Mining with R](https://www.tidytextmining.com/)
