...
book_words <- austen_books() %>%      # from the austen_books data frame
  unnest_tokens(word, text) %>%       # "tokenize" the words on the "text" column
  count(book, word, sort = TRUE)      # get the word count by book
book_words

total_words <- book_words %>%         # from the book_words data frame
  group_by(book) %>%                  # group rows by book
  summarize(total = sum(n))           # create a new column "total" and count the number of words
total_words

book_words <- left_join(book_words, total_words)    # join the "total" column to the book_words data frame
book_words

library(ggplot2)

ggplot(book_words,                                 # from the book_words data frame
       aes(n/total, fill = book)) +                # plot n/total, filled by book
...
freq_by_rank <- book_words %>%            # from the book_words data frame
  group_by(book) %>%                      # group rows by book
...
freq_by_rank %>%                                              # from the freq_by_rank data frame
...
rank_subset <- freq_by_rank %>%           # from the freq_by_rank data frame  
  filter(rank < 500,                      # filter rank to be lower than 500
...
book_words <- book_words %>%     # from the book_words data frame
  bind_tf_idf(word, book, n)     # apply the bind_tf_idf function
book_words

# As seen on the datasetm the inverse document frequency (idf) is very low (near zero) for words that occur in...
...
book_words %>%                   # from the book_words data frame
  select(-total) %>%             # select all the columns except the total column
  arrange(desc(tf_idf))          # sort it by tf_idf in descending order

book_words %>%                                                    # from the book_words data frame
  arrange(desc(tf_idf)) %>%                                       # sort it by tf_idf in descending order
  mutate(word = factor(word, levels = rev(unique(word)))) %>%     # add a new column "word" where each word corresponds to a factor 
  group_by(book) %>%                                              # group rows by book
...
physics_words <- physics %>%            # from the austen_books data frame
  unnest_tokens(word, text) %>%         # "tokenize" the words on the "text" column
...
plot_physics <- physics_words %>%                                     # from the physics_words data frame
...
plot_physics %>%                                        # from the plot_physcis data frame
...
# note: one interesting thing from the Einstein Plot is the "k" letter!

library(stringr)

physics %>%                                # from the physics data frame
  filter(str_detect(text, "_k_")) %>%      # filter the "text" column to contain the "_k_" string
  select(text)                             # select the "text" column      

# nte: the unnest_tokenS() function separates around punctuation like hyphens by defeault (e.g. "co" "ordinate")!
