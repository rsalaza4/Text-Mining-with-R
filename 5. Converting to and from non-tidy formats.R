...
# Note: only the non-zero values are included in the tified ouput!

# Perform snetiment analysis on these newspapers articles with the approach described in Chapter 2.
ap_sentiments <- ap_td %>%                                       # from the ap_td data frame
  inner_join(get_sentiments("bing"), by = c(term = "word"))      # get the inner join from the "word" column from the "bing" sentiment lexicon
ap_sentiments
...
inaug_tf_idf <- inaug_td %>%                         # from the inaug_td data frame
  ...
  arrange(desc(tf_idf))                              # arrange it in descending order by tf_idf
inaug_tf_idf
...
inaug_tf_idf %>%                                                   # from the inaug_tdf_idf data frame
  filter(document %in% c("1861-Lincoln",                           # filter the "document" column to contain 4 specific documents
                         ...
                         "1961-Kennedy",
                         "2009-Obama")) %>% 
  arrange(desc(tf_idf)) %>%                                        # arrange them in descending order by tf_idf
...
year_term_count %>%                                                                            # from the year_term_count data frame
  filter(term %in% c("good", "america", "foreign", "union", "constitution", "freedom")) %>%    # filter the "term" columnm to contain these words
...
# Construct a table with ine riw per document, including the metadata (such as is and datetimestamp)...
# as columns alongside tge text.
