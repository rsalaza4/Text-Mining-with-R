...
austen_books()
austen_bigrams <- austen_books() %>%                    # from the austen_books data frame
...
austen_bigrams %>%                 # from the austen_bigrams data frame
...
bigrams_separated <- austen_bigrams %>%                 # from the austen_bigrams data frame
  separate(bigram, c("word1","word2"), sep = " ")       # separate the "bigram" column into two columns: "word1" and "word2" separated by a space
bigrams_separated

bigrams_filtered <- bigrams_separated %>%               # from the bigrams_separated data frame
  filter(!word1 %in% stop_words$word) %>%               # filter for the words in "word1" column that are not included in the stop_words data frame
  filter(!word2 %in% stop_words$word)                   # filter for the words in "word2" column that are not included in the stop_words data frame
bigrams_filtered

# new bigram counts:

bigram_counts <- bigrams_filtered %>%                   # from the austen_filtered data frame
...
bigrams_united <- bigrams_filtered %>%                  # from the bigrams_filtered data frame
...
bigrams_filtered %>%                                    # from the bigrams_filtered data frame
...
bigram_tf_idf <- bigrams_united %>%                     # from the bigrams_united data frame
...
bigram_tf_idf %>%                                       # from the bigram_tf_idf data frame
...
bigrams_separated %>%                     # from the bigrams_separated data frame
...
not_words <- bigrams_separated %>%                 # from the bigrams_separated data frame
  filter(word1 == "not") %>%                       # filter "word1" column to contain the word "not"  
  inner_join(AFINN, by = c(word2 = "word")) %>%    # perform an inner join with the AFINN data frame by the "word2" column
  count(word2, value, sort = TRUE)                 # get the total count of the "word2" and "value" columns
not_words

not_words %>%                                            # from the not_words data frame
  mutate(contribution = n * value) %>%                   # add a new column "contribution" with n * value
  arrange(desc(abs(contribution))) %>%                   # sort the absolute value of contribution in descending order  
...
negated_words <- bigrams_separated %>%                   # from the bigrams_separated data frame
...
negated_words %>%                                                                 # from the negated_words data frame
...
austen_section_words <- austen_books() %>%        # from the austen_books data frame
...
# this let us count common pairs of words co-appearing within the same section (every 10 rows)
...
words_cors <- austen_section_words %>%          # from the austen_words data frame
...
words_cors %>%                                  # from the words_cors data frame
